...
clahe = cv2.createCLAHE(clipLimit=1.5, tileGridSize=(10,10)) #Create histogram
bd = 0.2 #border down of the sensitivity
bu = 5 #border up of the sensitivity
...
while(1):
    ...
    sens = (1-bd)*(255-mask)/255+bd
    print(sens)

    fgmask = np.absolute(frame-mask) # the difference of both of the frame

    fgmask_try = fgmask+35*((fgmask))*sens
    # fgmask_try = fgmask+127*(fgmask*sens/255)*(fgmask*sens/255)
    fgmask = fgmask.astype(np.uint8)
    fgmask_try = fgmask_try.astype(np.uint8)

    #fgmask = cv2.equalizeHist(fgmask)

    #fgmask = clahe.apply(fgmask)
    ...
    #threshold 1st step of detection
    ret,thresh1 = cv2.threshold(combine,200,255,cv2.THRESH_BINARY)


    thresh1 = cv2.morphologyEx(thresh1, cv2.MORPH_OPEN, kernel_open)
    thresh1 = cv2.morphologyEx(thresh1, cv2.MORPH_CLOSE, kernel_close)
    ...
    cv2.imshow('frame', frame)
    cv2.imshow('fgmask_try', fgmask_try)
    cv2.imshow('knn_mask', knn_mask)

    scipy.misc.toimage(frame, cmin=0.0, cmax=...).save(f'log_background/{order}.jpg')
